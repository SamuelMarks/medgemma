...
"""ModelRunner implementation using streaming async grpc to Triton model server.

Uses Triton GRPC aio client and relies on a model server running locally.
"""
...
class TritonStreamingServerModelRunner(model_runner.ModelRunner):
  ...
  async def _call(self, model_name, inputs, model_version, parameters):
    async with triton_aio.InferenceServerClient(_HOSTPORT) as client:
      ...
      # asyncio.CancelledError due to server-side cancellation.
